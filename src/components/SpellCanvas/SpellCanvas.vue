...
const onNodeClick = (e) => {
  if (state.selectedLink) {
    ...
    store.commit("spell/addConnection", { id, connection });
...
const removeNode = (node) => {
  ...
  for (const connection of state.connections) {
    if (connection.nodes.includes(node)) {
      ...
      store.commit("spell/removeConnection", { id: connection.id });
    }
  }
...
const collide = (node1, node2) => {
  let elements = [node1.getAttr("payload"), node2.getAttr("payload")];
...
const serialise = () => {
  console.log(store.getters['spell/getSpellSchema']);
  store.dispatch('spell/generateSpell');
};
</script>

<template>
  <div class="spell-canvas__wrapper">
    <button class="test" @click="serialise">TEST</button>
